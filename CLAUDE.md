...
我现在要做的就是做这个最简单的纯decoder的模型，就是输入的Boundary.csv的数据在最左边，相当于systerm prompt，然后因为可以输入第一瞬时的所有要预测的数据的状态，这里就最好是直接吧第一瞬时的数据作为输出数据。然后最后反向传播就是只传播需要预测的下一分钟的数据，并且是一一对应的。暂时现不考虑拓扑关系二十让模型自己内化到拓扑关系，现在暂时使用最基础的位置编码正弦–余弦的固定位置编码。
一次decoder的前向传播是这一个分钟预测到下一个分钟。使用multihead self attention
